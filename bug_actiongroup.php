...
# along with MantisBT.  If not, see <http://www.gnu.org/licenses/>.
	...
	foreach( $f_bug_arr as $t_bug_id ) {
		...
		case 'UP_FIXED_IN_VERSION':
			...
			if ( access_has_bug_level( config_get( 'update_bug_threshold' ), $t_bug_id ) ) {
				if ( $f_fixed_in_version === '' || ( version_get_all_rows( $t_project_id ) && version_get_id( $f_fixed_in_version, $t_project_id ) !== false ) ) {
					/** @todo we need to issue a helper_call_custom_function( 'issue_update_validate', array( $t_bug_id, $t_bug_data, $f_bugnote_text ) ); */
		...
		case 'UP_TARGET_VERSION':
			...
			if ( access_has_bug_level( config_get( 'roadmap_update_threshold' ), $t_bug_id ) ) {
				if ( $f_target_version === '' || ( version_get_all_rows( $t_project_id ) && version_get_id( $f_target_version, $t_project_id ) !== false ) ) {
					/** @todo we need to issue a helper_call_custom_function( 'issue_update_validate', array( $t_bug_id, $t_bug_data, $f_bugnote_text ) ); */
